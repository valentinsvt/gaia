...
<elm:container tipo="horizontal" titulo="Reporte de dotaciones">
    <div class="row">
        ...
        <div class="col-md-3">
            <g:select name="supervisor" id="supervisor" from="${Inspector.list([sort:'nombre'])}" optionKey="codigo" optionValue="nombre" noSelection="['-1':'Todos']" class="form-control input-sm"></g:select>
        </div>
        <div class="col-md-1">
...
<script type="text/javascript">
    $("#ver").click(function(){
        if($("#periodo").val()!="-1"){
            ...
            $.ajax({
                type    : "POST",
                url     : '${createLink(action:'dotacionesPorItem')}',
                data    : {
                    periodo : $("#periodo").val(),
    ...
    $("#imprimir").click(function () {
        if($("#periodo").val()!="-1"){
            var url = "${createLink(controller: 'reportesDotacion',action: 'dotacionesPorItemPdf')}?periodo="+$("#periodo").val()+"Wsupervisor="+$("#supervisor").val() ;
